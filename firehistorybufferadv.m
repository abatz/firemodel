function [rat,permrat,firehistory]=firehistorybuffer(totalarea,firehistory,option,nmem,Z,Z5,permrat,max_norecovery,nyeardrought);


% Z5 is the mean aridity from the past nyeardrought-years, this is used to immediately
% pull a percent of burned area out of rotation (forest --> non-forest)

% max_norecovery: what is the maximum extent of burned area from (1+nyeardrought)-yrs ago that does not

% scales linear function of aridity in past nyeardrought-yrs
...
removedforest=max_norecovery*percent_norecovery.*firehistory(end-nyeardrought,:);

% burned area from (1+nyeardrought) years ago is modified
addrat=removedforest;
firehistory(end-nyeardrought,:)=firehistory(end-nyeardrought,:)-removedforest;
